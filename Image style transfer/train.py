...
# 定义模型的损失函数
def loss(sess, model):
# sess为tf.session，model为models.py中构建的VGG模型

    # 先计算内容损失函数
    # 获取定义内容损失的VGG-19层名称列表及权重
    ...
    return loss# 内容损失和风格损失的加权和损失


# 构建给定矩阵的格莱姆矩阵，用来衡量风格
def gram(x, size, deep):
# 其中，x为给定矩阵，size为矩阵行数与列数的乘积，deep为矩阵的信道数

    x = tf.reshape(x, (size, deep))# 改变shape为（size,deep）
    g = tf.matmul(tf.transpose(x), x)# 求(x^T)*x

    return g# 返回描述图像风格的格莱姆矩阵
...
def train():
    ...
    with tf.Session() as sess:
        ...
        for step in range(settings.TRAIN_STEPS):
            ...
            if step % 50 == 0:
                print('step {} has been done.'.format(step))
